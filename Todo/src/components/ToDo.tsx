...
function ToDo({text, category, id}:IToDo) {
    ...
    const onClick = (event:React.MouseEvent<HTMLButtonElement>) => {
        ...
        setToDos(oldToDos => {
            ...
            const newToDoList = [...oldToDos.slice(0, targetIndex), newToDo, ...oldToDos.slice(targetIndex+1)];
    ...
    const deleteList = () => {
        setToDos(oldToDos => {
            const targetIndex = oldToDos.findIndex(toDo => toDo.id == id);
            const newToDoList = [...oldToDos.slice(0, targetIndex), ...oldToDos.slice(targetIndex+1)];
            localStorage.setItem("ToDos", JSON.stringify(newToDoList));
            return newToDoList;
        });
    };
    return (
        <ToDoList>
            ...
            <ButtonWrap>
                ...
                {category !== Categories.TO_DO && <ToDoButton name={Categories.TO_DO} onClick={onClick}>ToDo</ToDoButton>}
                <ToDoButton onClick={deleteList}>Delete</ToDoButton>
            </ButtonWrap>
        </ToDoList>
    )
}
